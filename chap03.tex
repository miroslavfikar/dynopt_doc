...
coresponding  final time $t_{f}$, \argfun{p}~-~scalar/vector of time
...
initial lengths (\argfun{optimparam.li}) euqal to $1/3$ were
...
profile \argfun{u}. They are ready to be ploted.\\  
The objective function at the optimal solution [\argfun{t},\argfun{u}]
is returned in before mentioned output structure \argfun{optimout} as
parameter \argfun{fval}:
\begin{verbatim}
...
been changed. For more informations about \argfun{options} and
\fun{dynopt} input and output arguments, see capter
\ref{cha:reference}. 
...
\end{verbatim}
where \argfun{ntimes} represents the density of the points ploted per
interval.
...
function \fun{process} and is valid for all afore mentioned user
functions.  
...
(\argfun{t}, \argfun{p}) are filled similary. For more informations
...
informations about \fun{objfun} definition, and its input and output
...
same as of \fun{process} but it is neccessary to tell \fun{dynopt} by
...
\argfun{Dceq} similar to those, descirbed in \fun{objfun}. For more
informations about \fun{confun} definition, and its input and output
arguments see chapter \ref{cha:reference}.
...
\fun{dynopt} performes minimisation of the objective
