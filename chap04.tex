...
constrained nonlinear minimisation routin~\fun{fmincon}. The chapter
...
\begin{description}
\item[\argfun{optvar}] -- The choise of optimisation variables: 1 -
  times, 2 - control, 2 - parameters. Their combination is given by
  their summations, e.g., 3 - otpimise times and control. All the
...
\item[\argfun{objtype}] -- Parameter which defines the type of
  ...
  \item[Sum type] - if Sum type objective function is used, parameter
    \argfun{objtype} is a structure containig two variables
...
\item[\argfun{ncolx}] -- Parameter which represents the number of
  collocation points for state variables. This has allways to be a
...
\item[\argfun{ncolu}] -- Parameter which represents the number of
  ...
  variables and also doesn't occure in \fun{process}, \fun{objfun},
  \fun{confun}. Otherwise it has to be a number greater than zero.
\item[\argfun{li}] --  Parameter representing lenghs of intervals. It
  has allways to be filled with ni--by--1 vector of initial lengths of
  intervals. 
...
Toolbox for all it's subroutines. They also present parameters usefull
...
\item [$\ve{M}$] -- a konstant mass matrix,
...
\item[\argfun{objfun}] The function to be minimised. \argfun{objfun}
  ...
  \item[Mayer form \eqref{eq:objmajer}] objective function is
    ...
    control variable(s), both evaluated at coresponding  final time
  ...
  \item[Sum form \eqref{eq:objsum}] objective function is evaluated in
    ...
    control variable(s), both evaluated at coresponding time $t_{i}$,
    scalar/vector \argfun{p}~-~time independent parameters,
    scalar/vector \argfun{xm}~-~measured variable(s) in the afore
...
\item[\argfun{confun}] The function that computes the nonlinear
  ...
  scalar~\argfun{t}~-~time value coresponding to the time $t$,
  scalar/vec\-tor \argfun{x}~-~state variable value(s), and
  scalar/vector \argfun{u}~-~control variable value(s) both
  coresponding to the value of \argfun{t}, scalar/vector
  ...
  \argfun{p}. 
\item[\argfun{process}] The function wich describes process model,
  ...
  coresponding to time \argfun{t}, and scalar/vector of time
...
By default~\fun{dynopt} will choose the large-scale algorithm if the
user supplies the gradient in objfun (and~\argfun{GradObj} is 'on'
in~\argfun{options}) and if only upper and lower bounds exists or only
...
\fun{dynopt} uses through the~\fun{fmincon} Sequential Programming
...
prepares plotable state and control profiles and \fun{constraints},
which prepares a user given equality and inequality plotable
...
number of points ploted per interval.
...
\argfun{ntimes} which defines the number of points ploted per interval.
